...
public class MapEditor extends JPanel implements ActionListener{
	...
	public void init() {
		...
		Dimension viewsize = new Dimension(c_width*Tileizer.WIDTH+18, c_height*Tileizer.WIDTH+18);
		...
		for (int i = 0; i<c_height; ++i) {
			for (int j = 0; j<s_width; ++j) {
				if (i*s_width+j>=TileMap.tiles.size()) {
					Tile temp = new Tile(TileMap.blank, j, i, 0);
					...
					Tile temp = new Tile(TileMap.icons.get(i*s_width+j), j, i, i*s_width+j);
					temp.setSize(Tileizer.WIDTH, Tileizer.WIDTH);
					selection.add(temp);
					mytiles[i][j] = temp;
					temp.setVisible(true);
		...
		for (int i = 0; i<tmap.mapdata.length; ++i) {
			for (int j = 0; j<tmap.mapdata[0].length; ++j) {
				Tile temp = new Tile(TileMap.icons.get(tmap.mapdata[i][j]), j, i, i*tmap.mapdata[0].length+j);
				temp.setSize(Tileizer.WIDTH, Tileizer.WIDTH);
				creation.add(temp);
				newtiles[i][j] = temp;
				temp.setVisible(true);
